      ...
      <div class="col-md-12">
        <div class="card card-info">
          <div class="card-header">
            <h3 class="card-title"><i class="nav-icon fas fa-edit"></i> Edit Product</h3>
          </div>

          <form class="form-horizontal" method = "POST" action = "{{ route('product.update', $data->product_id) }}" enctype = "multipart/form-data">
            @method('PUT')
            @csrf
            <div class="card-body">
                  ...
                  <div class="input-group mb-3">
                    ...
                    <select name="category_status" id="category_status" class="custom-select" required>
                      @foreach($categories as $category)
                        <option value= "{{ $category->category_id}}" {{  $data->product_type ==  $category->category_id ? 'selected' : ''}}>{{ $category->category_name }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
            ...
            <div class = "form-group row">
              <div class="col-md-12">
                ...
                <div class = "col-sm-12">
                    <textarea name = "product_information" class = "form-control" rows = "4">{{ $data->product_information}}</textarea>
                </div>
              </div>
            </div>
